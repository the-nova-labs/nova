...
def upload_file_to_github(target_protein: str, encoded_content: str):
    # Github configs
    ...
    target_file_path = os.path.join(github_repo_path, f'{target_protein}.txt')
    ...
    payload = {
        "message": f"Encrypted response for {target_protein}",
    ...
    if response.status_code in [200, 201]:
        ...
        bt.logging.error(f"Failed to upload file for {target_protein}: {response.status_code} {response.text}")
        return False
